---
title: Node.js
description: Explore examples of using KhulnaSoft AI SDK in your Node.js application.
---

# Node.js Examples

The following section will guide you through the various capabilities of the KhulnaSoft AI SDKs using minimal code examples.

<IndexCards
  cards={[
    {
      title: 'Generating Text',
      description: 'Generate text with Node.js.',
      href: '/examples/node/generating-text/',
    },
    {
      title: 'Generating Structured Data',
      description: 'Generate stuctured data with Node.js.',
      href: '/examples/node/generating-structured-data',
    },
    {
      title: 'Tools',
      description: 'Use tools with Node.js.',
      href: '/examples/node/tools',
    },
  ]}
/>
