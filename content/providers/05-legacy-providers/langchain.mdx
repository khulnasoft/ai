...
It takes this stream and uses KhulnaSoft AI SDK's [`StreamingTextResponse`](/docs/reference/stream-helpers/streaming-text-response)
to pipe text to the client and then KhulnaSoft AI SDK's `useChat` to handle the chat UI.
...
Then, we use the KhulnaSoft AI SDK's [`useChat`](/docs/reference/ai-sdk-ui/use-chat) method:
